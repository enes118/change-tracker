...
public class CdcConfigTemplateController {
    ...
    public ResponseEntity<Map<DbType, Map<String, String>>> getAllTemplates() {
        Map<DbType, Map<String, String>> templates = templateService.getAllActiveTemplates().stream()
                .collect(Collectors.toMap(
                        CdcConfigTemplateResponseDto::getDbType,
                        CdcConfigTemplateResponseDto::getTemplateMap
                ));
        return ResponseEntity.ok(templates);
    }

    @GetMapping("/details")
    public ResponseEntity<List<CdcConfigTemplateResponseDto>> getAllTemplateDetails() {
    ...
    public ResponseEntity<Map<String, String>> getTemplateByDbType(@PathVariable DbType dbType) {
        return templateService.getTemplateByDbType(dbType)
                .map(dto -> ResponseEntity.ok(dto.getTemplateMap()))
                .orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<CdcConfigTemplateResponseDto> createOrUpdateTemplate(@RequestBody CdcConfigTemplateRequestDto requestDto) {
        return ResponseEntity.ok(templateService.saveOrUpdateTemplate(requestDto));
    }
